...
@section('content')
          ...
          <div class="col-md-4 mb-3">
            ...
            <select name="pump_name" id="pump_name" class="form-select">
              ...
              @foreach ($pumpOptions as $pump)
                <option value="{{ $pump }}" {{ request('pump_name') == $pump ? 'selected' : '' }}>
                  {{ $pump }}</option>
              @endforeach
            </select>
          </div>
  ...
  @if ($days > 0)
    <div class="card mb-4">
      ...
      <div class="card-body">
        <p>Estimasi untuk rentang waktu <span class="text-primary">{{ $days }} hari</span>&nbsp;(sesuai filter
          tanggal Anda):</p>
        <div class="row">
          <div class="col-md-6">
  ...
  <div class="card">
    ...
    <div class="card-footer d-flex justify-content-center">
      {{-- Gunakan view pagination kustom --}}
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function exportData(format) {
        ...
        const exportUrl = format === 'excel' ? "{{ route('kontrol-riwayat-pompa-excel') }}" :
          "{{ route('kontrol-riwayat-pompa-pdf') }}";
